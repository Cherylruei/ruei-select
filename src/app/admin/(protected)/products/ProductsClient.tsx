...
export default function ProductsClient({ initialProducts, suppliers }: Props) {
  ...
  const filtered = filterSupplierId
    ? products.filter((p) => p.supplier_id === filterSupplierId)
    : products
    ...
    <>
      ...
      <div className='flex items-center justify-between gap-3 mb-5'>
        <div className='flex items-center gap-3'>
          ...
          <span className='text-[12.5px] text-[var(--neutral-400)]'>{filtered.length} 個商品</span>
        </div>
        <Link
      ...
      {filtered.length === 0 && (
        <div className='bg-white border border-[var(--neutral-200)] rounded-xl px-6 py-14 text-center'>
          ...
          <p className='text-[14px] font-medium text-[var(--neutral-600)] mb-1'>
            {filterSupplierId ? '此廠商尚無商品' : '尚未新增商品'}
          </p>
                      ...
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium ${
                          product.status === 'active'
                            ? 'bg-[var(--forest-50)] text-[var(--forest-deep)]'
                    ...
                    <td className='px-4 py-3 text-center hidden sm:table-cell'>
                      <button
                        ...
                        className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${
                          product.is_public ? 'bg-[var(--forest-base)]' : 'bg-[var(--neutral-300)]'
      ...
      {toast && (
        <div
          role='status'
          aria-live='polite'
          className={`fixed bottom-6 right-6 z-[300] px-5 py-3 rounded-xl shadow-lg text-[13px] font-medium text-white ${
            toast.type === 'success' ? 'bg-[var(--forest-base)]' : 'bg-[var(--color-error)]'
          }`}
        >
          {toast.message}
        </div>
